...
const ValidateLRN = ({
  configSchoolYear,
  setStudentLRN,
  displayValidateLrnModal,
  setDisplayValidateLrnModal,
  forProcessing,
  setForProcessing,
  lrnStat,
  setLrnStat,
      ...
      if (btnProcess === "Validate") {
        switch (lrnStat) {
          ...
          case LrnStatus.InSystemCurrentlyRegistered:
            window.location.href = `/registrationinfo/${validationLRN}`;
            /*setValMsg(
              `Your LRN was already registered for school year ${configSchoolYear}. Please enter another LRN.`
            );*/
            break;
          ...
          case LrnStatus.InSystemNotRegisteredCurrently:
            setValMsg(
              `Your LRN seems to be in the system, do you wish to register this LRN for school year ${configSchoolYear}?`
            );
          ...
          case LrnStatus.ValidButNotInSystem:
            setValMsg(
              `Your LRN is not yet in the system, do you wish to register this LRN for school year ${configSchoolYear}?`
            );
            setBtnProcess("Proceed");
            setForProcessing(true);
            setShouldShake(true);
  ...
  const setStudentLRNHandler = (e) => {
    ...
    if (forProcessing) {
      if (btnProcess === "Proceed") {
        //Close the modal and begin the registration entry
        setStudentLRN(validationLRN);
        setDisplayValidateLrnModal(false);
      } else if (btnProcess === "Yes") {
        //User is canceling the registration set the modal to initial state
        setStudentLRN("");
        setValidationLRN("");
      ...
      if (btnProcess === "Yes") {
        //Close the modal, the user doesn't want to register
        setStudentLRN("");
        setDisplayValidateLrnModal(false);
        setModalToInitialState();
      } else ValidateLrn();
  ...
  return (
    <div
      id="simpleModal"
      className={displayValidateLrnModal ? "modal" : "hidden"}
    >
